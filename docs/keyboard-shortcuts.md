...
| 内容を削除 | `Delete` / `Backspace` | `Delete` / `Backspace` | `Delete` / `Backspace` |
| 全選択 | `Cmd/Ctrl + A` | `Cmd/Ctrl + A` | `Cmd/Ctrl + A` |
| 操作履歴から復帰 | ツールバー「History」から任意地点へ | （なし） | （なし） |
...
| 行を削除 | `Cmd/Ctrl + -`（行選択時）/ コンテキストメニュー | `Cmd/Ctrl + -` | `Cmd/Ctrl + -` |
| 複数行を選択して削除 | 行ヘッダを `Shift + クリック` → `Cmd/Ctrl + -` | 対応 | 対応 |
| 複数行をまとめて挿入 | 複数行選択 → 右クリック「N行挿入」 | 対応 | 対応 |
...
| 列を削除 | `Cmd/Ctrl + -`（列選択時）/ 列メニュー | `Cmd/Ctrl + -` | `Cmd/Ctrl + -` |
| 複数列を選択して削除 | 列ヘッダを `Shift + クリック` → `Cmd/Ctrl + -` | 対応 | 対応 |
| 複数列をまとめて挿入 | 複数列選択 → 列メニュー「N列追加」 | 対応 | 対応 |
| 列名を変更 | 列ヘッダをダブルクリック / 列メニュー | （ダブルクリック） | （ダブルクリック） |
...
| 複数行選択（Shift+ヘッダークリック） | 対応 | 対応 | 対応 |
| 複数列選択（Shift+ヘッダークリック） | 対応 | 対応 | 対応 |
| 操作履歴パネル（任意地点へ復帰） | 対応（ツールバー「History」） | （バージョン履歴のみ） | （なし） |
