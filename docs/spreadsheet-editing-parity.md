...
- ✅ **フェーズ 3**: type-to-edit（印字文字で編集開始・内容置換）、セル右クリックメニュー
  ...
  オートフィル（`Cmd/Ctrl+D` 下方向フィル ＋ フィルハンドルのドラッグ、**等差数列の系列継続**対応）
- ⬜ 未着手（フェーズ4）: 非連続マルチレンジ選択（A-3）、日付・曜日など数値以外の系列推定、
  範囲内 Enter/Tab ラップ入力（C-3）、挿入貼り付け（D-2）、カットのマーチングアンツ（D-3）

> **フェーズ4の見送り理由**: これらはいずれもグリッド中核の選択・編集フロー
> （`selectedRange` 単一前提の描画/コピー/統計/削除、`updateCell` を含む編集経路）に踏み込む
> 変更で、GUI 実機での回帰確認が前提となる。特に非連続マルチレンジ（A-3）は
> `selections: CsvSelection[]` への段階移行を要する大規模改修。日々の単一セル編集を壊さないよう、
> 実機検証とセットで別ラウンドで実施する。
